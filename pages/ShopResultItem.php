...
// $conn = new mysqli('localhost', 'root', 'josh', 'foodie') or die("Could not connect to the database");
...
// echo"ssss";
if ($shop_message != '') {
$stmt = $pdo->prepare("INSERT INTO mem_message (message_id,name,shop_message,message_date,is_delete,shop_id,member_id)
      VALUES (:message_id,:name,:shop_message,:message_date,:is_delete,:shop_id,:member_id)");

$stmt->bindParam(':message_id', $message_id);
$stmt->bindParam(':name', $name);
$stmt->bindParam(':shop_message', $shop_message);
...
// //    die();
//     include '../common/connectFoodie.php';
// $stmt2 = $pdo->prepare("INSERT INTO report (report_id,content,is_delete,create_date,member_id,shop_id,message_id)

//       VALUES (:report_id,:content,:is_delete,:create_date,:member_id,:shop_id,:message_id)");

// $stmt2->bindParam(':report_id', $report_id);
// $stmt2->bindParam(':content', $shop_message);
// $stmt2->bindParam(':is_delete', $is_delete);
// $stmt2->bindParam(':create_date', $message_date);
// $stmt2->bindParam(':member_id',$member_id);
// $stmt2->bindParam(':shop_id',$shop_id);
// $stmt2->bindParam(':message_id',$message_id);
            ...
            <div class="ShopResultItem-InfoTop">
                <div class="ShopResultItem-InfoTitle">
                    ...
                    <div class="ShopResultItem-Score">
                        ...
                        ?>
                        <ul>
                            <li class="ShopResultItem-bb">總評價：<span class="ShopResultItemSpan"><?php echo $prod["scorea"]; ?></span> /
...
?>
                    <li class="ShopResultItem-bb">此攤商還沒有人做評分噢</li>
<?php
                }
                } catch (PDOException $ex) {
                    echo "錯誤原因 : ", $ex->getMessage(), "<br>";
...
<?php
                        ...
                        // 連到這個資料夾
                        // require_once("../common/connectFoodie.php");
                        $sql = "select score from shopscore where score_id = :mem_id and shop_id = :shop_id";
                        $products = $pdo->prepare($sql);
                        ...
                        while ($prod = $products->fetch(PDO::FETCH_ASSOC)){
                        ?>
                            <li class="ShopResultItem-zz" id="ShopResultItemzz"><?php echo $prod["score"]; ?></li>
                <?php
                }//while
        ...
        <div class="ShopResultItem-Infomation col-sm-10 col-xs-10">
            ...
            <div class="ShopResultItem-Message">
                ...
                try {
                    $sql = "SELECT m.img, m.memarea,s.* from mem_message s join member m WHERE m.name = s.name AND s.is_delete = '0' order by message_date DESC limit 0,2";
                    ...
                    while ($mem_messageRow = $mem_message->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        <div class="ShopResultItem-MessagePlace">
                            <div class="ShopResultItem-MessageImage">
                                <img src="../assets/images/shopmem/<?php echo $mem_messageRow["img"]; ?>" alt="留言大頭貼"/>
                            </div>
                            ...
                            <form action="" method="post">
                                <div class="ShopResultItem-MessageCheck">
                                 ...
                                 <!--    <button id="report" onclick="display_alert()" type="submit" class="message">檢舉</button> -->
                                </div>
                            </form>
                ...
                if ($_SESSION['mem']['member_id'] == "") {
                    ...
                    echo "<img src='../assets/images/shopmem/mem08.jpg'>";
                    echo "</div>";
...
<script type="text/javascript">
    ...
    function shopinit() {
        ...
        var z1 = document.getElementById("ShopResultItemzz");
        zz = z1.innerHTML;
        // console.log(zz);
        if(zz==5){
